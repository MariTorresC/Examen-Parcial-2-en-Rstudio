...
# Comprobación e instalación de librerías necesarias para análisis financiero y de series temporales
...
# Descarga histórica del índice IBEX 35 y cálculo de retornos logarítmicos diarios


# Extrae y limpia la serie de precios de cierre (elimina NAs)
start_date <- as.Date("2018-01-01")
end_date   <- as.Date("2025-05-08")
getSymbols("^IBEX", src = "yahoo", from = start_date, to = end_date)

close_prices <- na.omit(Cl(IBEX))

# Crea un data.frame con fechas y precios de cierre, apto para graficar o procesar
df <- data.frame(
  date  = as.Date(index(close_prices)),
  close = as.numeric(close_prices)
)

# Cálculo de los retornos logarítmicos diarios y fechas asociadas
log_returns <- diff(log(df$close))
fechas_log_returns <- df$date[-1]   

# Elimina posibles NA de los retornos y ajusta el vector de fechas correspondiente
valid_idx <- !is.na(log_returns)
log_returns <- log_returns[valid_idx]
fechas_log_returns <- fechas_log_returns[valid_idx]

# === 3. ANÁLISIS EXPLORATORIO ===
# Visualización inicial: precios y retornos logarítmicos diarios

# Gráfico de la evolución diaria del precio de cierre del IBEX 35
...
# Gráfico de la serie diaria de retornos logarítmicos del IBEX 35 
...
# Tests de heterocedasticidad sobre los retornos: ACF, PACF, ARCH-LM y Ljung-Box sobre retornos al cuadrado

# Análisis de autocorrelación en los retornos al cuadrado (posible presencia de efectos ARCH)
...
# Test estadístico de Engle para detectar heterocedasticidad tipo ARCH
arch_test <- ArchTest(as.numeric(log_returns), lags = 10)
print("ARCH-LM Test:"); print(arch_test)

# Test de Ljung-Box aplicado a los retornos al cuadrado (comprobar autocorrelación en la varianza)
...
# Búsqueda automática del modelo GARCH(p,q) óptimo según AIC en todas las combinaciones entre p, q = 0,1,2
...
# Muestra los modelos probados ordenados por AIC, para identificar el óptimo
print(results[order(results$AIC), ])

best_p <- results$p[which.min(results$AIC)]
best_q <- results$q[which.min(results$AIC)]

# Imprime el orden del modelo GARCH(p,q) óptimo según AIC
...
show(fit_opt)   # Imprime el resumen del modelo GARCH óptimo ajustado
...
# Visualización de la volatilidad condicional diaria estimada por el mejor modelo GARCH escogido

# Grafica la volatilidad condicional estimada por el modelo GARCH óptimo a lo largo del tiempo
...
# Extracción de coeficientes estimados por el modelo óptimo para cálculos adicionales (p.ej. varianza para el día siguiente)
...
# Cálculo de la volatilidad condicional mínima estimada sobre todo el periodo analizado
...
# Cálculo del VaR diario (Value at Risk) usando el modelo ajustado y comparación con el método tradicional
...
# Gráfico comparativo entre la volatilidad condicional diaria estimada y la volatilidad histórica global

vol_cond <- sigma(fit_opt)  # volatilidad condicional diaria
vol_hist <- sd(log_returns)   # volatilidad histórica (constante)

# Extrae los valores extremos (máximo y mínimo) de la volatilidad condicional y sus fechas
...
ggplot(df_vol, aes(x = Fecha, y = Volatilidad)) +
  ...
  # Anota en la gráfica los días con volatilidad condicional máxima y mínima
...
# Comparación del VaR diario usando diferentes medidas de volatilidad: histórica, mínima y máxima modelo ARCH/GARCH
...
# Ajuste adicional de GARCH(1,1) clásico si el modelo óptimo no lo es (para efectos comparativos)
...
# Ajuste de modelo ARCH(1), comparación frente a GARCH(1,1) en términos de volatilidad condicional
...
# Comparación del VaR diario obtenido de ambos modelos para el último dato disponible
...
# Obtención de residuos, volatilidad y estandarización de residuos del modelo óptimo, con tablas resumen
...
stopifnot(length(sigma_t) == length(ht), 
          length(ht) == length(et), 
          length(et) == length(z_t), 
          length(z_t) == length(fechas_log_returns))
...
resultados <- data.frame(
  ...
  Varianza            = ht,
  Residuos            = et,
  ResiduosEstandarizados = z_t
)

rownames(resultados) <- NULL

ultimas_fechas <- tail(fechas_log_returns, 5)
ultimos_ht     <- tail(ht, 5)
ultimos_et     <- tail(et, 5)
ultimos_z_t    <- tail(z_t, 5)
...
print(data.frame(Fecha = ultimas_fechas, h_t = ultimos_ht), row.names = FALSE)

# --- Residuos crudos (no estandarizados) ---
et <- residuals(fit_opt)
cat("   Residuos del modelo (et), últimos 5 valores:\n")
print(data.frame(Fecha = ultimas_fechas, et = ultimos_et), row.names = FALSE)

# --- Residuos Estandarizados ---
z_t <- et / sigma_t
cat("   Residuos estandarizados (z_t), últimos 5 valores:\n")
print(data.frame(Fecha = ultimas_fechas, z_t = ultimos_z_t), row.names = FALSE)

# === 16. Simulación de 50 trayectorias de retornos (modelo óptimo) ===
# Simulación Monte Carlo de trayectorias de retornos con el modelo óptimo ajustado (1000 caminos, muestra 50)
...
# Visualización tipo Fan Chart de medias y percentiles (ICs) de retornos simulados multivariados en horizonte de 10 días
...
# Histograma del retorno acumulado a 10 días en las simulaciones de Monte Carlo
...
# === 19. Pronósticos ===
# Predicción de retornos, volatilidad y precios del IBEX 35 para los próximos 10 días usando el modelo GARCH óptimo,
# más visualización agregando histórico del último año y bandas de confianza
...
cat(sprintf("Pronóstico para el 09/05/2025:\nRetorno esperado: %.6f\nVolatilidad esperada: %.6f\n", 
            predicted_return, predicted_volatility))

# Calcula el precio de cierre pronosticado para el próximo día usando el retorno esperado y el último precio observado
...
# Genera una secuencia de fechas para los próximos 10 días de pronóstico
fecha_inicio <- as.Date("2025-05-08")
...
# -----------------------------------------------
# Graficar pronóstico para los próximos 10 días + un historico de un año
# -----------------------------------------------

# Definir fechas finales y de corte
fecha_final <- as.Date("2025-05-08")
