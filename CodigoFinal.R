...
cat(sprintf("Portafolio: $%s\n", format(valor_portafolio, big.mark = ",")))
...
# Calcular precios proyectados usando el último precio observado
last_price <- tail(df_precios$close, 1)
...
df_forecast <- data.frame(
  ...
  Precio_Pronosticado = predicted_prices,
  Volatilidad = predicted_volatilities
)

print(df_forecast)

# Graficar precios pronosticados
plot(
  df_forecast$Fecha, df_forecast$Precio_Pronosticado, type = "l",
  col = "blue", lwd = 2, xlab = "Fecha", ylab = "Precio proyectado",
  main = "Pronóstico de Precios para los Próximos 10 Días"
)
...
fecha_final <- as.Date("2025-05-06")
...
ggplot(df_plot, aes(x = Fecha, y = Precio)) +
  ...
  labs(
    title = "Pronóstico de precios con intervalo de confianza (último año + 10 días)",
    x = "Fecha", y = "Precio"
  ) +
  theme_minimal(base_size = 13)